...
export function getParentRentSeriesList(parentContexts: { parentKey: string; badgeLabel: string }[], selectedBeds: number[]): SeriesInfo[] {
    const multibed = selectedBeds.length > 1;
    const series: SeriesInfo[] = [];
    for (const ctx of parentContexts) {
        for (const beds of selectedBeds) {
            const bedEntry = BED_KEYS.find((b) => b.beds === beds)!;
            series.push({
                key: multibed ? `${ctx.parentKey}:${beds}` : ctx.parentKey,
                label: multibed ? `${ctx.badgeLabel} avg · ${bedEntry.label}` : `${ctx.badgeLabel} avg`,
                color: "#9ca3af",
                dash: "6 4",
            });
        }
...
export function formatWeekLabel(dateStr: string): string {
    return formatDate(dateStr, { utc: true, style: "monthDay" });
}
...
/** Short label for the latest aggregated bucket, e.g. "latest month" for MoM. */
...
export function buildMultiAreaRentData(
    ...
    parentSeries?: { key: string; rows: TrendRow[] }[],
    ...
    const aggregatedParents: Record<string, TrendRow[]> = {};
    for (const parent of parentSeries ?? []) {
        aggregatedParents[parent.key] = aggregateTrendRows(parent.rows, granularity);
    }
    ...
    for (const parent of parentSeries ?? []) {
        for (const beds of selectedBeds) {
            (aggregatedParents[parent.key] ?? []).filter((r) => r.beds === beds).forEach((r) => allWeeks.add(r.week_start));
        }
    }
    return Array.from(allWeeks)
        ...
        .map((w) => {
            ...
            for (const parent of parentSeries ?? []) {
                for (const beds of selectedBeds) {
                    const key = multibed ? `${parent.key}:${beds}` : parent.key;
                    const row = (aggregatedParents[parent.key] ?? []).find((r) => r.beds === beds && r.week_start === w);
                    if (row) point[key] = Math.round(row.median_rent);
                }
            }
            return point;
...
export type SalesMetric = "median_price" | "avg_cap_rate" | "listing_count";

export const SALES_METRIC_VALUES: SalesMetric[] = ["median_price", "avg_cap_rate", "listing_count"];

export function isSalesMetric(v: string): v is SalesMetric {
    return (SALES_METRIC_VALUES as string[]).includes(v);
}

/** Y-axis view for the sales chart: percent change from the first point vs. absolute value. */
export type SalesYView = "pct" | "abs";

export const SALES_Y_VIEW_VALUES: SalesYView[] = ["pct", "abs"];

export function isSalesYView(v: string): v is SalesYView {
    return (SALES_Y_VIEW_VALUES as string[]).includes(v);
}
...
function rollupSalesTrendGroup(group: SalesTrendRow[], bucketMonthStart: string): SalesTrendRow {
    ...
    const totalCapRateCount = group.reduce((s, r) => s + (r.cap_rate_count ?? 0), 0);
    const sortedPrices = [...prices].sort((a, b) => a - b);
    const sortedCapRates = [...capRates].sort((a, b) => a - b);
    const medianCapRate = capRates.length > 0 ? medianFromSorted(sortedCapRates) : null;
    return {
        month_start: bucketMonthStart,
        median_price: medianFromSorted(sortedPrices),
        avg_cap_rate: medianCapRate,
        listing_count: totalCount,
        cap_rate_count: totalCapRateCount,
    };
}
...
/** Aggregate monthly SalesTrendRows by calendar quarter (median of monthly medians, sum counts, median cap rates). */
...
export function buildMultiAreaSalesData(
    ...
    parentSeries?: { key: string; rows: SalesTrendRow[] }[],
): Array<Record<string, string | number>> {
    ...
    const aggregatedParents: Record<string, SalesTrendRow[]> = {};
    for (const parent of parentSeries ?? []) {
        aggregatedParents[parent.key] = aggregateSalesTrendRows(parent.rows, granularity);
    }
    ...
    for (const parent of parentSeries ?? []) {
        (aggregatedParents[parent.key] ?? []).forEach((r) => allBuckets.add(r.month_start));
    }
    ...
    return Array.from(allBuckets)
        ...
        .map((bucket) => {
            ...
            for (const parent of parentSeries ?? []) {
                const row = (aggregatedParents[parent.key] ?? []).find((r) => r.month_start === bucket);
                if (row) {
                    const val = row[metric];
                    if (val != null) point[parent.key] = typeof val === "number" ? val : Number(val);
                }
            }
            if (minListing !== null) {
