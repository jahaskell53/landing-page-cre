...
export const PropertyMap = ({
    ...
    const mapStyle = useMapboxStyle("standard");
    ...
    // Map initialisation (recreate when theme style changes)
    useEffect(() => {
        if (!mapContainer.current) return;

        const previousCenter = map.current?.getCenter();
        const previousZoom = map.current?.getZoom();
        ...
        const m = new mapboxgl.Map({
            ...
            style: mapStyle,
            center: previousCenter ? [previousCenter.lng, previousCenter.lat] : (initialCenter ?? [-122.4194, 37.7749]),
            zoom: previousZoom ?? initialZoom ?? 10,
        });
        map.current = m;
        ...
        m.on("load", () => {
            ...
            m.on("click", UNCLUSTERED_LAYER, (e) => {
                ...
                const popupHref =
                    props.isReit && props.buildingZpid ? `/analytics/building/${encodeURIComponent(props.buildingZpid)}` : `/analytics/listings/${props.id}`;
        ...
        const showSelectedPopup = () => {
            ...
            const selectedPopupHref =
                property.isReit && property.buildingZpid
                    ...
                    : `/analytics/listings/${property.id}`;
