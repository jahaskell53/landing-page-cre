...
/**
 * Composer shell for chat input. Vendored from the prompt-kit chat set. Holds the
 * input value (controlled or uncontrolled) and submit handler in context so the
 * textarea and action buttons stay in sync.
 */
function PromptInput({ className, isLoading = false, maxHeight = 240, value, onValueChange, onSubmit, children }: PromptInputProps) {
    ...
    return (
        <TooltipProvider>
            ...
            >
                <div className={cn("rounded-3xl border border-input bg-background p-2 shadow-xs", className)}>{children}</div>
            </PromptInputContext.Provider>
        </TooltipProvider>
    );
}
...
function PromptInputTextarea({ className, onKeyDown, disableAutosize = false, ...props }: PromptInputTextareaProps) {
    ...
    useEffect(() => {
        ...
        textarea.style.height =
            typeof maxHeight === "number" ? `${Math.min(textarea.scrollHeight, maxHeight)}px` : `min(${textarea.scrollHeight}px, ${maxHeight})`;
